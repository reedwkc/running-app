...
export function computeAnalysisMetrics(streams, laps, targetHRFloor, isTreadmill, profile, isContinuousEffort, isTrail){
  ...
  // Same terrain reasoning as everywhere else trailRun gates pace-derived inference
  // (session-trends.js, chat.js's qualifiesTier2) - a trail run's speed at a given HR is
  // genuinely slower for reasons that have nothing to do with fitness, so this formula
  // (which assumes flat-ground speed) would misread terrain as a fitness ceiling. Now
  // known at computation time via the pre-import cardTrailOverride toggle (week-view.js),
  // not just excluded downstream after a wrong preview already displayed.
  if(profile && profile.maxHR && !isContinuousEffort && !isTrail){
...
export async function selectStravaCandidate(id, activityId){
  ...
  try{
    ...
    // Read here (BEFORE analysis runs), not applied downstream after the fact - the whole
...
async function runStravaAnalysis(activity, streams, structureDesc, target, isTreadmill, realLaps, sessionType, isTrail){
  const system = [
  ...
  const userText = "Activity: "+(activity?activity.name:'unknown')+", "+(activity?activity.distance_km:'?')+"km, "+(activity?Math.round(activity.moving_time_min):'?')+"min.\n"+
    ...
    "Target HR zone floor for the main effort: "+(target.hr||'not pace-specific')+(target.paceCeiling?(', pace CEILING '+target.paceCeiling+' - this is a Zone 2 session governed by HR, not a pace target. Any pace at or under the ceiling is correct however far under it is; do NOT treat running slower than the ceiling as underperformance. Only exceeding it is a finding.'):(target.pace?(', target pace '+target.pace):''))+".\n"+
  ...
  const metrics = computeAnalysisMetrics(streams, boundaries, targetHRFloor, isTreadmill, state.profile, parsed.continuousEffort, isTrail);
