...
export async function generateProfileContext(){
  ...
  "VO2max (S5) sessions specifically show TWO markers, not one, because HR realistically climbs across a whole set of reps (not just within one rep) from combined HR-kinetics lag and real cardiac drift rep-to-rep - this runner's own logged sessions already show that exact pattern. The hollow marker ("+computeVO2maxBuildStartHR()+"bpm, ~88% Max HR) is a realistic opening-rep target; the solid marker ("+computeOptimalHR({},'S5')+"bpm, ~95% Max HR) is what the LAST 1-2 reps of the set should build toward. If asked about 'the optimal HR' on a VO2max card, or why a number they saw felt too high to sustain, be explicit that the solid number is a final-reps ceiling, not a flat target to hold from rep one - treating it that way is exactly how a session blows up early. "+
