...
import { resolveProbePace } from './session-ceiling.js';
import { computeInjuryRiskWarnings } from './injury-tracking.js';
import { applyInjuryStatusBlock, getActiveReturnToRun, stripInjuryStatusBlock } from './return-to-run.js';
...
import { fmtDuration, fmtPace, fmtPaceExact, fmtTime, formatMinutesToClock, parsePaceLabelToSec, timeAgo } from '../lib/format.js';
...
// The GOAL IMPACT instruction used to unconditionally tell the model to connect every event
// to the active GOAL-slot goal, with no way to know that goal's own training block hasn't
// actually started yet (a real gap after finishing a DIFFERENT, now-archived goal - not a
// continuation, see loadPreviousCompletedGoalCardData in goal-trajectory.js). That produced
// exactly the reported bug: a skipped rest-or-walk day getting a manufactured "costs nothing
// toward sub-1:30:00" verdict against a goal with zero training behind it yet. hmNotStarted
// comes straight from buildTrajectoryPrompts's own block-start check, so this and the visible
// GOAL TRAJECTORY block above it always agree on whether the block has actually begun.
function goalImpactInstruction(hmNotStarted, subjectPhrase){
  if(hmNotStarted){
    return ' Also, before the VERDICT SUMMARY block, add a block on its own line starting with exactly "GOAL IMPACT:" followed by exactly 1 short sentence: since '+activeGoalPhrase()+'\'s training block has not started yet (see context above), do not invent a cost or contribution claim against it - say plainly that training toward that goal hasn\'t begun yet, so '+subjectPhrase+' doesn\'t affect it either way, framing it instead against the current actual context (e.g. recovery/wind-down following the previously completed goal) if that\'s relevant.';
  }
...
export async function autoCoachMessage(kind, data){
  ...
  if(kind==='profile'){
    ...
    const {trajectoryContext:fwTrajCtx, trajectoryPrompt:fwTrajPrompt, trajectory10KPrompt:fwTraj10K, hmNotStarted:fwHmNotStarted} = await buildTrajectoryPrompts();
    prompt = 'I did an activity that wasn\'t part of the prescribed plan: '+JSON.stringify(data.obj)+'.'+conversationNote+swapNote+fwTrajCtx+' Write 2-4 sentences: judge how this fits alongside the actual plan - does it meaningfully add to this week\'s cumulative load (worth factoring into recovery expectations for the next prescribed session), is it low-stakes enough to just note and move on, or does it raise a genuine flag (pain mentioned, RPE surprisingly high for what was described, conflicts with what\'s coming up)? Don\'t manufacture a concern if there isn\'t one - plenty of extra easy activity is perfectly fine to just acknowledge. If this genuinely changes what the next few days of the actual plan should look like - adding recovery, adjusting an upcoming session\'s intensity because of accumulated load - end with a block starting on its own line with exactly "PASTE TO REBUILD:" followed by 1 complete sentence stating what should change, written so I can copy it into the main Claude conversation - only include this when a real change is warranted.'+fwTrajPrompt+fwTraj10K+goalImpactInstruction(fwHmNotStarted, 'this specific activity')+' Finally, always end with a block on its own line starting with exactly "VERDICT SUMMARY:" followed by exactly 1 short sentence stating just the core essentials, condensed, for a compact summary card separate from your fuller reply above.';
  } else if(kind==='skip'){
    ...
    const {trajectoryContext:skipTrajCtx, trajectoryPrompt:skipTrajPrompt, trajectory10KPrompt:skipTraj10K, hmNotStarted:skipHmNotStarted} = await buildTrajectoryPrompts();
    ...
    const {trajectoryContext, trajectoryPrompt, trajectory10KPrompt, hmNotStarted} = await buildTrajectoryPrompts();
    ...
    // What the session had LEFT - the free final rep, where the plan scheduled one
    // (coach/session-ceiling.js). The only evidence in this whole prompt that speaks to the
    // CEILING rather than the cost: a prescribed-pace session run to target shows what that
    // pace cost in heart rate, never what was still in reserve.
    //
    // Deliberately built OUT here rather than inside the Tier-estimate block below, which
    // only fires on a Strava-verified or treadmill-qualifying session. A ceiling reading is
    // worth having in the coach's hands whether or not the session cleared that bar.
    const prescribedPaceForSet = data.obj.prescribedPaceSec || null;
    ...
    // classifyActualEffort (lib/effort.js) - the same classifier free/swapped-workout
    // trend-feeding uses (coach/session-trends.js, ui/modals.js) - reads the session's
    // hardest real work-lap HR against LTHR bands to say whether it was actually threshold
    // or actually VO2max effort, not just "hard or not": a free workout can be "easy, long,
    // sub-threshold, threshold, or VO2max," each needing to feed the right model, not a
    // single flat hard/not-hard bucket (2026-08-29).
    const actualEffort = (!isThresholdOrVo2 && !isGoalPaceLong) ? classifyActualEffort(data.obj, state.profile) : null;
    const isDataDrivenThresholdEffort = actualEffort==='threshold';
    const isDataDrivenVo2Effort = actualEffort==='vo2max';
    const isDataDrivenHardEffort = isDataDrivenThresholdEffort || isDataDrivenVo2Effort;
    ...
    if(qualifiesTier2 || qualifiesTier3){
      ...
      const isVo2Session = data.day.type==='vo2max' || isDataDrivenVo2Effort;
      ...
      const goalLongNote = isGoalPaceLong
        ...
        ? (' Today\'s session wasn\'t planned as threshold/VO2max, but the logged data itself shows a genuine '+(isDataDrivenVo2Effort?'VO2max-or-harder':'at-or-above-threshold')+' effort ('+(isDataDrivenHardTreadmillEffort ? ('RPE '+data.obj.rpe+' on a treadmill') : ('a real work-lap HR at or above '+(isDataDrivenVo2Effort?'LTHR+6bpm':'LTHR-8bpm')+' - genuinely '+(isDataDrivenVo2Effort?'VO2max':'threshold')+'-band effort, not just "hard"'))+') - treat it as real Tier '+tierNum+' evidence for '+(isVo2Session?'vo2maxPaceSec':'ltPaceSec')+' specifically, on its own terms, the same as if it had been planned that way. Don\'t discount it just because it doesn\'t match what was scheduled - what matters is what the effort actually was, not what the plan called it.')
        : '';
      ...
      // trim this app already applies (computeAnalysisMetrics's effI0 - waits for HR to
      // cross the ZONE'S FLOOR and hold 15s, see strava-import.js) only excludes the
      // pre-effort ramp, not the continued climb THROUGH the rest of the zone once past the
      // floor. For a rep this short, a real chunk of even the "trimmed" window can still be
      // HR climbing toward its true ceiling for that pace - understating what the pace
      // actually costs at a genuinely settled effort, and reading as a bigger undershoot
      // than it really was. This is the same ramp-lag principle already stated in general
      // terms above ("matters most for short reps... less for longer threshold reps") - this
      // ties it explicitly to the NUMBER for whatever this session's own rep length actually
      // is, not a vague "short reps somewhere."
      const workLapDurations = (tierNum===2 && data.obj.stravaImport && Array.isArray(data.obj.stravaImport.laps))
        ? data.obj.stravaImport.laps.filter(l=>l.role==='work' && l.durationSec).map(l=>l.durationSec)
        : [];
      const shortestWorkLapMin = workLapDurations.length ? Math.round(Math.min(...workLapDurations)/6)/10 : null;
      const shortRepCaveat = (shortestWorkLapMin!=null && shortestWorkLapMin < 6)
        ? (' Also specific to this number: today\'s work reps are short (as brief as ~'+shortestWorkLapMin+' min) - even after excluding the pre-effort ramp (timeToTargetSec), HR very likely kept climbing toward its true ceiling for the rest of a rep this short, so the reported avgHR still understates what that pace actually costs at a genuinely settled effort. Treat a fast pace at seemingly-easy HR from reps this short as WEAKER evidence than the same read from a longer (10+ minute), genuinely sustained effort - don\'t nudge ltPaceSec as confidently from this alone, and weight it less than corroborating evidence from longer efforts if the two conflict.')
        : '';
  ...
  try{
    ...
    let textResp = (dataResp.content||[]).filter(b=>b.type==='text').map(b=>b.text).join('\n') || 'Sorry, I could not generate a response.';
    // Applied and stripped BEFORE the reply is rendered, not with the other blocks below -
    // every other block is machine plumbing the runner never needed to see, but this one
    // changes what the app will let them train, so it must be acted on even if a later
    // parsing step throws, and its raw JSON must never reach the chat bubble.
    await applyInjuryStatusBlock(textResp);
...
export async function requestTierEstimateFallback(tierNum, day, obj, weekN){
  ...
  // Same data-driven read as the main tier prompt (autoCoachMessage) - a swap/extra whose
  // planned day wasn't vo2max can still BE vo2max-effort evidence; this is the retry path
  // for when the main reply dropped the block, so it must classify the same way or a
  // data-driven VO2max session could silently fall back to updating ltPaceSec instead here.
  const isVo2 = day.type==='vo2max' || classifyActualEffort(obj, state.profile)==='vo2max';
  const vo2maxPaceAnchorVal = (anchor.vo2maxPaceSec!=null) ? anchor.vo2maxPaceSec : await computeVO2maxPaceSec();
...
export async function buildRecentTimeline(days){
  ...
  function attachWorkout(entry, day, type, eq){
    ...
    // A swapped session (e.g. a 10K time trial run in place of a scheduled long run) has
    // its own real name on the entry itself - day.name is only what was ORIGINALLY
    // scheduled, and using it unconditionally here misrepresented what was actually run
    // to the coach in general conversation (same bug already fixed for the weekly summary
    // in weekly-summary.js's summaryLines mapper, never applied here).
    const effName = type==='bike' ? bikeSessionName(eq?eq.kind:null)
      : (entry.swappedForName || entry.name || (entry.performedAlt==='alt' && day.alt ? day.alt.name : day.name));
    if(entry.skipped){
...
export async function generateProfileContext(){
  ...
  // The single most important fact in this whole prompt when it's true, so it's computed
  // deterministically and stated as fact rather than left to be inferred from a pain note
  // buried in a session log. Before this existed, the background paragraph below asserted a
  // hardcoded "nothing currently active" no matter what the runner had reported, and the
  // only return-to-training guidance available was keyed on silence (see inactivityNote
  // below), which reads a ten-day injury layoff as "normal week-to-week variation".
  let injuryNote = '';
  let activeReturnToRun = null;
  try{
    activeReturnToRun = await getActiveReturnToRun();
    if(activeReturnToRun && activeReturnToRun.caps){
      const rtr = activeReturnToRun;
      const inj = rtr.injury;
      injuryNote = "\nACTIVE INJURY - this overrides ordinary training guidance. "+rtr.note+
        " These limits are computed deterministically by the app from the injury's own duration and severity (coach/return-to-run.js), and the plan validator enforces them - do not talk the runner past them, and never tell them to just follow the plan as written while this is active."+
        " The app has a 'Adjust the plan for this' button on the injury banner that restructures the remaining block around these limits - point the runner at it rather than describing a manual edit in prose."+
        " Returning from injury means volume comes back before intensity: easy running rebuilds first, quality work returns last, and the long run is the single session most likely to re-injure, so it rebuilds on its own slower curve than weekly total."+
        (rtr.phase==='resting' ? " Running has not resumed at all yet, so anything in the plan before it does is not a session to prepare for - it is a session that needs removing." : "")+
        " Be honest about what this costs the goal if it costs something; do not manufacture reassurance, and do not manufacture doom either.";
    }
  }catch(e){}
  let inactivityNote = '';
  try{
  ...
  // The gauge card's own "Current fitness projects to..." text is computed deterministically
  // (getBestAvailableLTPace + projectedTimeFromLTPace) every time it renders - but this
  // general-conversation context never actually handed that number over, only the raw
  // per-tier dumps below and a separate LLM-synthesized headline (trajectoryNote, next
  // block) that can be stale or reflect a different tier's number than the one currently
  // ruling. Left to reconcile Tier 1/2/3 on its own, the coach could - and did - end up
  // citing a faster/less-authoritative tier's LT pace than the one actually driving the
  // number the runner sees on screen. This hands over the exact same figure as ground truth.
  let projectionNote = '';
  try{
    const best = await getBestAvailableLTPace();
    if(best.ltPaceSec!=null){
      const goals = (state.goalConfig||defaultGoalConfig()).activeGoals||[];
      const lines = goals.filter(g=>g.zoneKey==='GOAL'||g.zoneKey==='RACE10K').map(g=>{
        const distanceKm = g.distanceKm || (g.zoneKey==='RACE10K'?10:21.0975);
        const projSec = projectedTimeFromLTPace(best.ltPaceSec, distanceKm);
        return (g.label||g.zoneKey)+(g.raceName?(' ('+g.raceName+')'):'')+(g.raceDate?(', '+g.raceDate):'')+': current fitness projects to roughly '+formatMinutesToClock(projSec/60)+' over '+distanceKm+'km'+(g.goalTimeLabel?(' - goal is '+g.goalTimeLabel.toLowerCase()):'');
      });
      if(lines.length){
        projectionNote = "\nCurrent race-time projection (the SAME deterministic calculation the front-page gauge bar's own \"Current fitness projects to...\" text uses, not an LLM estimate - computed from whichever tier is currently best-available, right now that's "+best.source+" at "+fmtPaceExact(best.ltPaceSec)+" LT pace): "+lines.join(' | ')+". This is the authoritative, current answer to any \"will I hit sub-X\" or race-day achievability question - state it plainly and stay consistent with it, rather than independently re-deriving your own LT-pace read from the raw per-tier numbers below (those exist so you can explain WHY this number is what it is if asked, not so you can recompute a different one from them).";
      }
    }
  }catch(e){}
  let trajectoryNote = '';
  ...
  // tiers of evidence, and tierNote further down explains it can be superseded by a fresher
  // Tier 2/3 read for actual pace targets and race-time projections. Scoping "authoritative,
  // never override" to HR alone (not bundling the paired pace into the same blanket claim)
  // avoids contradicting that later, more nuanced instruction and projectionNote's own
  // deterministic best-available-tier figure - a real source of the coach citing an
  // inconsistent LT pace when discussing race-day achievability.
  ...
  (()=>{
    ...
    // This sentence used to hardcode "but nothing currently active", and then instructed the
    // coach not to caution about injury history that isn't active - a static string that kept
    // asserting the runner was healthy while they were reporting an injury, and actively told
    // the coach to drop the subject. Same failure class as the threshold-pace prompt that
    // fell out of sync with the code: a claim about live state, frozen into a literal. It now
    // reads the real state (see injuryNote above, computed from coach/return-to-run.js).
    const injuryBackground = activeReturnToRun
      ? "Background: history of ankle/thigh/quad issues, AND SOMETHING IS CURRENTLY ACTIVE - see the ACTIVE INJURY section, which is authoritative over anything else here. "
      : "Background: history of ankle/thigh/quad issues, none currently active on record - if pain comes up, the runner will report it directly and the plan adjusts in real time from that; don't proactively caution about injury history that isn't currently active. If the runner tells you in this conversation that something IS hurting, that is exactly the situation the INJURY STATUS block exists for - use it rather than only noting the pain in prose. ";
    const background = injuryBackground+"Forest trails are paused for now by preference, not medical necessity. Mileage increases capped at 10%/week (standard ramp-rate guidance)"+capExceptionNote+". ";
  ...
  // The wire that was missing: the runner said "my quad has hurt since the half marathon" in
  // this very chat, and nothing anywhere turned that sentence into state. UPDATE INSIGHTS
  // wrote it into a prose paragraph, which no deterministic check can read - so the plan kept
  // prescribing intervals and 14km long runs, and the coach kept saying to follow it. A
  // reported injury has to become structured state, not a remembered sentence.
  " Separately and with real weight: if the runner tells you in this conversation that something is hurting, that they are injured, that they cannot run, or that an injury they previously reported has now resolved, end your reply with a block on its own line starting with exactly \"INJURY STATUS:\" followed by a single valid JSON object and nothing else. To open or update an injury: {\"active\":true,\"bodyPart\":\"right quad\",\"severity\":\"pain\",\"startDate\":\"YYYY-MM-DD\",\"expectedReturnDate\":\"YYYY-MM-DD or null\",\"note\":\"the runner's own words, briefly\"}. To clear one: {\"active\":false}. severity must be exactly \"ache\", \"pain\" or \"injury\" - use \"ache\" only for a passing twinge the runner is plainly not worried about, \"pain\" for something genuinely affecting training, \"injury\" for something that has stopped them running. startDate should be when it actually started hurting (work it out from what they tell you, e.g. \"since the half marathon\" means the race date), not today's date, because the app sizes the entire return-to-running ramp from that date. Set expectedReturnDate only if they actually say when they expect to run again, otherwise null. This block writes real app state that changes prescribed paces, caps volume, holds quality work and unlocks the plan-restructure button - so emit it whenever a reported injury genuinely starts, changes or ends, and never emit it speculatively from a passing mention of ordinary training soreness. It is separate from and additional to UPDATE INSIGHTS and FOLLOW UPS; emit all of them when all apply, with INJURY STATUS last.";
