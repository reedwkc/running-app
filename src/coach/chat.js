...
export async function autoCoachMessage(kind, data){
  ...
  if(kind==='profile'){
    ...
    // What the session had LEFT - the runner's own reserve answer, and the free final rep
    // where the plan scheduled one (coach/session-ceiling.js). The only evidence in this whole
    // prompt that speaks to the CEILING rather than the cost: a prescribed-pace session run to
    // target shows what that pace cost in heart rate, never what was still in reserve.
    //
    // Deliberately built OUT here rather than inside the Tier-estimate block below. These come
    // from the runner, not a device, so they exist on a treadmill, on a watch-less run, and on
    // any session that never syncs to Strava - which is exactly when the app's measured signals
    // are thinnest and this is worth most. Gating it on Tier qualification (the first cut of
    // this) would have withheld it precisely when it was the only ceiling evidence available.
    const prescribedPaceForReserve = data.obj.prescribedPaceSec || null;
    let reserveTrend = null;
    try{
      const rr = await readJsonArray('reserve-history');
      if(rr.ok && rr.value.length) reserveTrend = interpretReserveTrend(rr.value.filter(p=>p.sessionType===data.day.type));
    }catch(e){}
    if(reserveTrend && reserveTrend.status==='insufficient') reserveTrend = null;
    const reserveDesc = describeReserve(data.obj.reserve);
    const reserveTrendRead = reserveTrend ? (' Across recent sessions of this type the reserve read is: '+reserveTrend.note) : '';
    const reserveNote = reserveDesc
