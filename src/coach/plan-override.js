...
import { applyPlanOverrides, buildWeeks, classifyReducedWeek, computeWeekPlannedKm, continuousTempo, fartlek, hillRepeats, ladderReps, vo2maxReps } from '../data/plan.js';
...
async function buildPlanOverrideSystemPrompt(opts){
  ...
  return [{type:'text', text:
    ...
    'HILLS SPECIFICALLY: this runner has said they are not always keen on hill work and expects to skip it "often." Whenever you prescribe a hill-repeats day, always include a "note" field on that day naming a concrete flat alternative with real numbers (e.g. "Not feeling the hill today? Same stimulus flat: Nx45s at threshold/VO2max pace, same recovery, logged as a swap") so there is always a real, ready substitute rather than the runner having to invent one or skip the session outright. Do not stop prescribing hill work entirely just because of this preference - it is a legitimate, real stimulus and the point is a graceful opt-out on a given day, not removing the format from the rotation.\n'+
