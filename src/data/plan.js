...
export const WHY = {
  easy:{why:'Keeps weekly volume high without adding fatigue - builds the aerobic base (capillary density, fat-burning efficiency) everything else is built on.',
        tip:'No pace target here on purpose - your route is uneven enough that one would mislead you more than help. HR and feel ARE the target, not a fallback for when you can\'t hit a number: if you can\'t speak in full sentences, you\'re running too hard for what this session is for, regardless of what the pace happens to read.'},
  threshold:{why:'Trains your body to buffer and clear lactate at faster paces - this directly raises the pace you can sustain for a half marathon. The main fitness driver in this plan.',
        tip:'Pace is the default target; HR is the tie-breaker whenever the two disagree. What that actually looks like, rep by rep: '
          +'HR reading BELOW the zone on rep 1 (sometimes rep 2) while your pace is correct - completely normal, that\'s cardiac lag, not a sign you\'re not working hard enough. Do NOT speed up to force the number higher; hold pace and let HR catch up on its own over the next 60-120 seconds. '
          +'HR sitting AT or ABOVE the top of the zone with reps still to go - this is the one that actually means something: ease off 5-10 sec/km for the rest of the set. It won\'t "settle" on its own if you keep grinding at the same pace; it'
          +'\'ll keep climbing. '
          +'HR only reaching the top of the zone on your LAST rep, even though pace and effort felt identical to earlier reps - normal fatigue drift, not a red flag by itself. '
          +'HR staying quiet (never really climbing into zone) for the WHOLE session despite holding correct pace throughout - also fine. Some days it just runs low; keep trusting pace as the real number rather than pushing harder to chase a reading that isn\'t coming. '
          +'Use rep 1 as your calibration read: hold the prescribed pace, check HR in the closing 30-60 seconds of that rep, then apply whichever rule above actually matches what you see.'},
