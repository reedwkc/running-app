...
export const WHY = {
  easy:{why:'Keeps weekly volume high without adding fatigue - builds the aerobic base (capillary density, fat-burning efficiency) everything else is built on.',
        tip:'HR and feel ARE the target here, not a fallback for when you can\'t hit a pace number - that holds wherever you run, not just on any one specific route. Terrain, weather, and how tired your legs already are all shift what "easy pace" should even mean from run to run; HR/effort adjusts to all of that automatically, a fixed pace number doesn\'t. If you can\'t speak in full sentences, you\'re running too hard for what this session is for, regardless of what the pace happens to read.'},
