...
// Reps are prescribed at S4 - the measured LT pace - EXACTLY, with no multiplier.
//
// Worth stating plainly, because "threshold" and "sub-threshold" get used interchangeably
// around this plan. True sub-threshold means sitting just under the second lactate turnpoint,
// which in its pure form needs a lactate meter to locate. Without one, the "measured LT pace"
// available here is itself HR-derived and already conservative relative to a real LT2 - so
// pinning the pace to it while targeting MID-zone HR rather than LTHR itself lands
// functionally sub-threshold, without claiming a precision the equipment cannot deliver.
// methodology-reference.js states the same thing as the method's documented deviation.
//
// A paceRatio parameter used to sit in this signature, defaulted to 0.989 by the recipe and
// then ignored entirely by the body. Removed rather than wired up: as a multiplier on a
// sec/km pace, 0.989 is 1.1% FASTER than LT - supra-threshold, which is precisely the
// deviation this method's own reference text names as breaking it. A dead parameter
// describing the opposite of the intended behaviour is a trap for whoever later decides to
// "fix" it by honouring it.
export function threshold(reps, repM, recoverySec, recoveryLabel, wuKm, cdKm){
...
export const WHY = {
  easy:{why:'Keeps weekly volume high without adding fatigue - builds the aerobic base (capillary density, fat-burning efficiency) everything else is built on.',
        tip:'Run this by HR and feel, staying in Zone 2 - the pace shown is a ceiling, not something to aim at. There is no such thing as too slow here, and no credit for being closer to the ceiling. What actually changes day to day is the pace that keeps you in Z2 at all: terrain, heat, wind, and how tired your legs already were will all move it, sometimes by a lot, and following them down is the session working rather than a bad day. Going OVER the ceiling is the one real way to get this wrong - that is how an easy day quietly turns into a medium one, and medium days are what wreck a block like this. The plainest check still applies: if you can\'t speak in full sentences, you\'re running too hard for what this session is for, whatever the pace reads.'},
  threshold:{why:'Trains your body to buffer and clear lactate at faster paces - this directly raises the pace you can sustain for a half marathon. The main fitness driver in this plan.',
        tip:'Pace is the default target; HR is the tie-breaker whenever the two disagree. The zone itself is a RANGE, and sitting comfortably inside it - mid-zone especially - is the normal, correct, expected state for most of each rep, not something to react to. Here\'s what actually calls for a reaction, rep by rep: '
          +'HR still BELOW the zone on rep 1 (sometimes rep 2) while your pace is correct - completely normal, that\'s cardiac lag, not a sign you\'re not working hard enough. Do NOT speed up to force the number higher; hold pace and let HR catch up on its own over the next 60-120 seconds. '
          +'HR pinned at the very CEILING of the zone (its top edge) or over it, with reps still to go - this is the one that actually means something: ease off 5-10 sec/km for the rest of the set. It won\'t drift back down on its own if you keep grinding at the same pace; it\'ll keep climbing. '
          +'HR only reaching that ceiling on your LAST rep, having sat comfortably mid-zone at the same pace earlier - normal fatigue drift, not a red flag by itself. '
          +'HR staying low (never really settling into the zone at all) for the WHOLE session despite holding correct pace throughout - also fine. Some days it just runs quiet; keep trusting pace as the real number rather than pushing harder to chase a reading that isn\'t coming. '
          +'Use rep 1 as your calibration read: hold the prescribed pace, check HR in the closing 30-60 seconds of that rep, then match whichever case above actually fits.'},
  vo2max:{why:'Raises your aerobic ceiling (VO2max), which gives your threshold pace more room to climb over time.',
        tip:'Pace is the target here, full stop - not HR. This intensity never reaches a real steady state within one rep, so HR is always playing catch-up and will never fully reflect effort until you\'re several reps deep. What that means in practice: '
          +'HR reading low - even well below the zone - on rep 1 or rep 2 while pace is correct is completely expected. It does NOT mean you\'re under-cooking it, and speeding up to chase the number higher is exactly what wrecks the back half of the set. Pace hitting the number is the only real confirmation you need early on. '
          +'HR should climb across the WHOLE set, not within a single rep - expect it noticeably lower on the opening 1-2 reps, building toward the marked gauge value by your last 1-2. That rising arc across the session is what correct pacing looks like here, not a flat or immediately-maxed HR line. '
          +'The one real warning sign is pace fading on reps 3-4 versus reps 1-2 - that means you went out too hot early, which HR won\'t clearly tell you about until it\'s already too late to fix that rep. Hold even pace across all reps and let HR do whatever it does.'},
  long:{why:'Builds the durability and fuel efficiency to hold pace deep into a half marathon. On progressive-finish weeks, it also rehearses running under fatigue - exactly what the closing kilometers feel like.',
        tip:'Two different rules in one session. The Zone 2 base is run by HR and feel - the pace shown for it is a ceiling, not a target, and there is no such thing as too slow. Expect that same pace to cost more HR late in the run than it did in the first hour; if holding it pushes you out of Z2, let the pace go and stay in the zone, because the aerobic work is the point of that portion. The faster finish segments are real pace targets - run those to their numbers. Shifting into any faster finish zone: ease into it over the first minute rather than jumping straight to that zone\'s target pace. HR will lag the pace change by 60-120 seconds, so a low reading the moment you shift gears just means HR hasn\'t caught up yet, not that you\'re short of the right effort - give it that full settling window before judging. Only if HR is STILL clearly under the zone after that window has passed is it worth easing the pace back a touch; don\'t react to the first 60-120 seconds of a fresh zone.'},
  race:{why:'This is the goal effort itself.',
        tip:'Early (first 2-3km): run by pace and feel, not HR - it lags at the start and will read artificially low regardless of real effort, so trusting it here just means starting too conservatively. Middle: HR becomes a genuinely useful cross-check once it\'s caught up - if it\'s tracking meaningfully above your usual goal-pace HR well before halfway, that\'s a real signal to ease back slightly now rather than push through, since a good race rarely starts with HR already elevated at goal pace this early. Late: HR running near max in the closing kilometers is expected and correct in a race, not a warning sign the way it would be in training - this is the one context where "HR is very high" is exactly what should be happening, nothing to react to except holding on.'}
};
...
{ n:1, dates:'Aug 3-9', cutback:false,
  callout:null,
  days:[
    {tag:'Wed - Aug 5', name:'Threshold', zone:'S4', type:'threshold', data:threshold(6, 1000, 90,'jog',2,1.5)},
...
{ n:2, dates:'Aug 10-16', cutback:false, callout:'Second quality day added on Monday, starting this week - a genuine threshold session in its own right, still shorter than Wednesday\'s but no longer a token effort.',
  days:[
    {tag:'Mon - Aug 10', name:'Threshold (shorter)', zone:'S4', type:'threshold', data:threshold(4, 1000, 90,'jog',1.5,1.5)},
...
{ n:3, dates:'Aug 17-23', cutback:false, callout:'Pre-race peak week before the 10K taper.',
  days:[
    {tag:'Mon - Aug 17', name:'Threshold (shorter)', zone:'S4', type:'threshold', data:threshold(5, 800, 90,'jog',1.5,1.5)},
    {tag:'Wed - Aug 19', name:'Threshold', zone:'S4', type:'threshold', data:threshold(6, 1200, 90,'jog',2,1.5)},
...
{ n:6, dates:'Sep 7-13', cutback:false, callout:'Peak week. Saturday\'s long run is the single most important session of the block.',
  days:[
    {tag:'Mon - Sep 7', name:'Threshold (shorter)', zone:'S4', type:'threshold', data:threshold(5, 1000, 90,'jog',1.5,1.5)},
...
{ n:7, dates:'Sep 14-20', cutback:true, callout:'Taper begins.',
  days:[
    {tag:'Mon - Sep 14', name:'Threshold (shorter)', zone:'S4', type:'threshold', data:threshold(3, 800, 120,'jog/walk',1.5,1.5), note:'Taper week - kept light.'},
    {tag:'Wed - Sep 16', name:'Threshold', zone:'S4', type:'threshold', data:threshold(4, 1000, 150,'jog/walk',2,1.5), note:'Full recovery - sharpen, don\'t grind.'},
...
export const SESSION_RECIPES = {
  threshold: a => threshold(a.reps, a.repM, a.recoverySec, a.recoveryLabel||'jog', a.wuKm, a.cdKm),
