...
export const WHY = {
  ...
  race:{why:'This is the goal effort itself.',
        tip:'Early (first 2-3km): run by pace and feel, not HR - it lags at the start and will read artificially low regardless of real effort, so trusting it here just means starting too conservatively. Middle: HR becomes a genuinely useful cross-check once it\'s caught up - if it\'s tracking meaningfully above your usual goal-pace HR this early, that\'s worth noting, not necessarily panicking about. Late: HR running near max in the closing kilometers is expected and correct in a race, not a warning sign the way it would be in training - this is the one context where "HR is very high" is exactly what should be happening.'}
};
