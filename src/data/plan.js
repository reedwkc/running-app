...
export const WHY = {
  easy:{why:'Keeps weekly volume high without adding fatigue - builds the aerobic base (capillary density, fat-burning efficiency) everything else is built on.',
        tip:'Run to the pace band, and treat the whole band as fair game rather than aiming at its quick end. That range is not slack - it is what absorbs the things that genuinely change what "easy" costs on any given day: terrain, heat, wind, and how tired your legs already were at the start. On a hilly or hot day the slow end IS the correct answer, and holding the quick end anyway is how an easy day quietly turns into a medium one. HR is the secondary check on exactly that: if you are already at the slow end of the band and HR is still climbing out of the zone, the day is telling you to go easier still. And the oldest check is the plainest one - if you can\'t speak in full sentences, you\'re running too hard for what this session is for, whatever the pace reads.'},
  ...
  long:{why:'Builds the durability and fuel efficiency to hold pace deep into a half marathon. On progressive-finish weeks, it also rehearses running under fatigue - exactly what the closing kilometers feel like.',
        tip:'Each segment below carries its own pace target - run to those. The base portion gets the same latitude as an easy run: it is a target, not a rule, and drifting a few seconds either side of it over the hours is normal. Expect the base pace to cost more HR late in the run than it did in the first hour; that is the session doing its job, not a reason to slow down. Shifting into any faster finish zone: ease into it over the first minute rather than jumping straight to that zone\'s target pace. HR will lag the pace change by 60-120 seconds, so a low reading the moment you shift gears just means HR hasn\'t caught up yet, not that you\'re short of the right effort - give it that full settling window before judging. Only if HR is STILL clearly under the zone after that window has passed is it worth easing the pace back a touch; don\'t react to the first 60-120 seconds of a fresh zone.'},
